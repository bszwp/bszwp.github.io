...
​	get请求携带的参数直接追加到url后面即可。? 后面跟的就是url。
...
​	post请求的的参数是在send方法中携带，参数的格式必须是：xxx=XXX&yyy=YYY格式。注意，参数中的中文必须要经过url编码。

```javascript
xhr.send('user='+encodeURI(nameVal) + '&pass=' + passVal);
```
